...
function formatSortedTableData(rawdata, content) {
	if (content == "time") { // in seconds
		...
		return rawdata.toLocaleString(language, {minimumFractionDigits: 2, maximumFractionDigits: 2});
...
function getFormattedTD(rawdata, rawdata_details, is_table, column_classes) {
	...
	if (rawdata_details != null) {
		...
		if ("content" in rawdata_details
			...
			var formatedData = formatSortedTableData(rawdata, rawdata_details["content"]);
			if ("unit" in rawdata_details) {
				td.html(formatedData + " " + rawdata_details["unit"]);
			} else {
				td.html(formatedData);
			}
		...
		// Apply text-align if necessary
		if ("align" in rawdata_details && (rawdata_details["align"] == "right" || rawdata_details["align"] == "center")) {
			td.css("text-align", rawdata_details["align"]);
		}
...
function SortedTable(table, rawdata_details, rawdata) {
	...
	this.init = function() {
		...
		for (var i=0 ; i!=head_columns.length ; i++) {
			...
			if (i == 0
					...
					|| ("order" in this.rawdata_details[i-1] && this.rawdata_details[i-1]["order"])
					|| ("content" in this.rawdata_details[i-1]
	...
	this.update = function(order_on_column) {
		...
		if (order_on_column == -1) {
			...
			for (var i=0 ; i!=head_columns.length ; i++) {
				var previous_order = $(head_columns[i]
						.getElementsByTagName("a")[0]).attr("data-sortedtable-order");
				if (previous_order == "asc" || previous_order == "desc") {
			...
			var previous_order = $(head_columns[order_on_column]
					.getElementsByTagName("a")[0]).attr("data-sortedtable-order");
			new_order = previous_order == "asc" ? "desc" : "asc";
			for (var i=0 ; i!=head_columns.length ; i++) {
